...
async def _send_log_task(text: str):
    if str(LOG_CHANNEL_ID).startswith("-100"):
        try: await bot.send_message(LOG_CHANNEL_ID, f"{pe('📝')} <b>BOT LOG:</b>\n{text}", parse_mode='HTML')
        except Exception: pass
...
async def get_stats(user_id: int, view_type: str) -> Optional[dict]:
    ...
    if "matches" in u and "solo" not in u:
        u["solo"] = {k: v for k, v in u.items() if k not in ["_id", "user_id", "coins", "teddy_count", "heart_count", "username", "first_name", "booster_expiry"]}
...
async def update_stats_mongo(summary: dict, game_type: str):
    try:
        for p in summary['players']:
            ...
            if not u_doc: u_doc = {"user_id": p['id'], "coins": 0, "teddy_count": 0, "heart_count": 0, "username": "", "first_name": ""}
            
            if "matches" in u_doc and "solo" not in u_doc:
                u_doc["solo"] = {k: v for k, v in u_doc.items() if k not in ["_id", "user_id", "coins", "teddy_count", "heart_count", "username", "first_name", "booster_expiry"]}
...
async def process_admin_buy(user_id: int, item: str, action: str) -> bool:
    if action == 'app':
        ...
        refund = 3000 if item == 'booster' else GIFT_CONFIG['amount']
...
# 🎴 IMPROVED VISUAL PLAYER CARD GENERATOR (FUT STYLE) BIGGER SIZE
def generate_stats_card_image(s: dict, game_type: str) -> Optional[io.BytesIO]:
    ...
    try:
        ...
        try:
            ...
            f_m = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 28)
            f_s = ImageFont.truetype("DejaVuSansMono.ttf", 24)
        except:
            f_title = f_l = f_m = f_s = ImageFont.load_default()

        # Tier Logic
        runs = s.get('total_runs', 0)
        tier = "BRONZE"; theme_color = (205, 127, 50)
        if runs >= 1000: tier = "DIAMOND"; theme_color = (185, 242, 255)
        elif runs >= 500: tier = "GOLD"; theme_color = (255, 215, 0)
        elif runs >= 100: tier = "SILVER"; theme_color = (192, 192, 192)

        # Draw Outer Border
        try:
            draw.rounded_rectangle([(20, 20), (width-20, height-20)], radius=20, outline=theme_color, width=6)
            draw.rounded_rectangle([(40, 110), (width-40, 210)], radius=15, fill=(30, 35, 45))
            draw.rounded_rectangle([(40, 230), (width//2-10, 480)], radius=15, fill=(30, 35, 45))
            draw.rounded_rectangle([(width//2+10, 230), (width-40, 480)], radius=15, fill=(30, 35, 45))
        except AttributeError:
            draw.rectangle([(20, 20), (width-20, height-20)], outline=theme_color, width=6)

        # Header
        title = f"⭐ {game_type.upper()} CAREER STATS ⭐"
        draw.text((width//2, 65), title, font=f_title, fill=theme_color, anchor="mm")
        
        # Player Info
        name = s.get('username') or s.get('first_name', 'Player')
        draw.text((60, 160), f"@{name}", font=f_l, fill=(255, 255, 255), anchor="lm")
        draw.text((width-60, 160), f"TIER: {tier}", font=f_l, fill=theme_color, anchor="rm")
        
        # Batting Box (Left)
        draw.text((width//4+15, 270), "BATTING", font=f_m, fill=(88, 255, 33), anchor="mm")
        draw.text((60, 320), f"Runs: {s.get('total_runs', 0)}", font=f_s, fill=(200,200,200))
        draw.text((60, 370), f"Highest: {s.get('highest_score', 0)}", font=f_s, fill=(200,200,200))
        total_balls = s.get('total_balls', 0)
        sr = "0.00" if total_balls == 0 else f"{(s.get('total_runs', 0) / total_balls) * 100:.2f}"
        draw.text((60, 420), f"Strike Rate: {sr}", font=f_s, fill=(200,200,200))
        
        # Bowling Box (Right)
        draw.text((width*3//4-15, 270), "BOWLING", font=f_m, fill=(255, 33, 170), anchor="mm")
        draw.text((width//2+30, 320), f"Wickets: {s.get('total_wickets', 0)}", font=f_s, fill=(200,200,200))
        b_balls = s.get('total_bowling_balls', 0)
        r_conceded = s.get('total_runs_conceded', 0)
        eco = "0.00" if b_balls == 0 else f"{(r_conceded / max(1,b_balls)) * 6:.2f}"
        draw.text((width//2+30, 370), f"Economy: {eco}", font=f_s, fill=(200,200,200))
        matches = s.get('matches', 0)
        win_rate = (s.get('wins',0)/max(1, matches))*100
        draw.text((width//2+30, 420), f"Win Rate: {win_rate:.1f}%", font=f_s, fill=(200,200,200))
        
        # Footer
        draw.text((width//2, 540), "ONE CLUB. ONE SPIRIT. ENDLESS GLORY.", font=f_s, fill=(100, 150, 255), anchor="mm")
...
async def ai_cb_handler(call):
    ...
    game.last_update = now # Update activity time
    ...
    if cmd == 'diff':
        ...
        if user_won:
            text += "🎉 <b>You won the toss!</b> Choose to Bat or Bowl:"
        ...
        out = (bat_num == bowl_num) # 🏏 WICKET LOGIC ONLY! (No 6 ball limit)
        if not out:
        ...
        if end_innings:
            if game.innings == 1:
                ...
                if is_tie: txt += f"\n\n{pe('🤝')} <b>Match Tied!</b>"
                    ...
                    txt += f"\n\n{pe('😔')} <b>BOT WON!</b> (+5 Coins)"
                    await add_coins(uid, 5, "AI Match Loss")
...
async def buy_cmd(message):
    ...
    rows = [
        ...
        [{"text": '2x Coin Booster (3k)', "callback_data": "buy_item:booster", "style": "success"}]
    ]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
...
async def stats_cmd(message):
    ...
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    img_buf = await asyncio.to_thread(generate_stats_card_image, s, 'solo')
...
async def tourstats_cmd(message):
    ...
    img_buf = await asyncio.to_thread(generate_stats_card_image, s, 'tour')
...
async def cb_statsview(call):
    ...
    await bot.answer_callback_query(call.id)
    try: 
        img_buf = await asyncio.to_thread(generate_stats_card_image, s, view_type)
...
async def cb_buy_item(call):
    ...
    cost = 3000 if item == 'booster' else GIFT_CONFIG["amount"]
...
async def cb_admin_buy(call):
    ...
    try:
        if action == 'app': await bot.send_message(user_id, f"🎉 <b>Congratulations!</b>\n\nYour request for <b>{item.capitalize()}</b> has been {pe('✅')} <b>Approved</b>!", parse_mode='HTML')
...
async def handle_all_messages(message):
    ...
    if message.chat.type == 'private' and message.from_user.id in WAITING_FOR_GIFT_AMOUNT:
        ...
        if message.text and message.text.strip().lower() == 'skip':
            return await bot.reply_to(message, f"{pe('✅')} Skipped. Amount not changed.")
        elif message.text and message.text.strip().isdigit():
            GIFT_CONFIG['amount'] = int(message.text.strip())
            return await bot.reply_to(message, f"{pe('✅')} Shop amount updated to {GIFT_CONFIG['amount']} Coins.")
        else:
            return await bot.reply_to(message, f"{pe('❌')} Invalid amount. Skipped.")
    
    if message.from_user.id in TOUR_SETUP_STATE and message.chat.type == 'private':
        state = TOUR_SETUP_STATE[message.from_user.id]
        if state["step"] == 1:
            if not message.text: return await bot.reply_to(message, f"{pe('⚠️')} Send GC ID (e.g., -100123456).")
            try:
                ...
                await bot.reply_to(message, f"{pe('❌')} Invalid ID. Send only numbers.")
            return
    ...
    if message.chat.type == 'private' and message.from_user.id in WAITING_FOR_ADS:
        ...
        await bot.reply_to(message, f"{pe('📢')} Broadcasting your message everywhere. This will take some time...")
        try:
            chats = await db.known_chats.find().to_list(length=None)
            success = 0
            for c in chats:
