...
class Player:
    ...
    history: List[str] = field(default_factory=list) # 🔥 Tracks ball-by-ball history
...
BAN_DURATION = timedelta(minutes=5) # 🔥 Changed to 5 Minutes
...
def ball_commentary(runs: int, lang: str) -> str: 
    return random.choice(COMMENTARY['en'].get(runs, [f"{runs} runs!"]))
...
def live_score_text(game: Game) -> str:
    ...
    for idx, p in enumerate(game.players):
        marker = ""
        if get_batter(game) and p.id == get_batter(game).id:
            marker = "🏏 "
        elif get_bowler(game) and p.id == get_bowler(game).id:
            marker = "🎯 "
...
def format_scoreboard_text(game: Game) -> str:
    title = f"{pe('🏆')} TOUR FINAL SCORE {pe('🏆')}" if game.is_special else f"{pe('🎖️')} SOLO FINAL SCORE {pe('🎖️')}"
    total_runs, total_balls = sum(p.runs for p in game.players), sum(p.balls for p in game.players)
    overs_str = f"{total_balls // 6}.{total_balls % 6}"
    text = f"┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓\n       {title}\n┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫\n\n"
    for p in game.players:
        sr = f"{(p.runs / p.balls) * 100:.1f}" if p.balls > 0 else '0.0'
        b_balls, b_wkts, b_runs = game.bowling_balls.get(p.id, 0), game.wickets_by_bowler.get(p.id, 0), game.bowling_runs.get(p.id, 0)
        eco = f"{(b_runs / b_balls) * 6:.1f}" if b_balls > 0 else '0.0'
        
        out_marker = " ❌ (Elim)" if p.is_eliminated else ""
        text += f"{pe('👤')} <b>{mention_user(p)}</b>{out_marker} : <b>{p.runs}</b> ({p.balls})\n"
        text += f"  ➥ {pe('💥')} 4s: {p.fours} | 6s: {p.sixes} | SR: {sr}\n"
        text += f"  ➥ ⚾ Bowl: {b_balls}b | {b_wkts}W | {b_runs}R | Eco: {eco}\n\n"
    text += f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛"
    return text
    ...
    try:
        ...
        if media_doc and "file_id" in media_doc:
            ...
            try:
                ...
                print(f"Mongo File_ID send failed for {filename}, falling back to local file. Error: {e}")
...
async def eliminate_bowler(chat_id: int):
    try:
        ...
        if count == 1:
            ...
            # 🔥 Start round immediately to resume game
...
async def eliminate_batter(chat_id: int):
    try:
        ...
        if count == 1:
            batter.runs -= 6
            batter.balls += 1
            batter.history.append('0')  # Skipped ball counts as 0 run
            ...
            batter.balls += 1
            batter.history.append('W') # Eliminated
...
async def cb_bowl(call):
    ...
    if game.is_special or getattr(game, 'is_tour_match', False):
        ...
        if len(history) >= 2 and history[-1] == num and history[-2] == num:
            ...
            if batter: 
                batter.runs += 1 
                batter.history.append('1') # wide run
...
async def process_shot(chat_id: int):
    ...
    if bat == bowl:
        ...
        batter.history.append('W') # 🔥 Added to history
        ...
        batter.history.append(str(bat)) # 🔥 Added to history
...
async def end_game(chat_id: int):
    ...
    # 🔥 OLD format removed, using `/score` format
    
    coin_msg = f"\n💰 <b>Coins Earned (x2 Boost!):</b>\n"
    ...
    # 🏆 FORWARD SCOREBOARD TO SCORE CHANNEL 🏆
...
async def stats_cmd(message):
    ...
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    await bot.reply_to(message, format_stats_profile_html(s, 'solo'), parse_mode='HTML', reply_markup=kbd)
...
async def gift_cmd(message):
    run_safe_track(message)
    if not is_owner(message.from_user.id): return await bot.reply_to(message, "❌ Only Owner can edit gift.", parse_mode='HTML')
...
@bot.message_handler(commands=['cachemedia'])
async def cachemedia_cmd(message):
    if not is_owner(message.from_user.id): return
    await bot.reply_to(message, "⏳ <b>वीडियो अपलोड और MongoDB में सेव हो रहा है... कृपया 1-2 मिनट इंतज़ार करें।</b>", parse_mode='HTML')
    all_files = [
        'zero.mp4', 'zero2.mp4', 'zero3.mp4', 'one.mp4', 'one2.mp4', 'one3.mp4',
        'two.mp4', 'two2.mp4', 'two3.mp4', 'three.mp4', 'three2.mp4', 'three3.mp4',
        'four.mp4', 'four2.mp4', 'four3.mp4', 'five.mp4', 'five2.mp4', 'five3.mp4',
        'six.mp4', 'six2.mp4', 'six3.mp4', 'out.mp4', 'out2.mp4', 'out3.mp4', 'duck.mp4',
        'start.mp4', 'ball.mp4', 'bat.mp4', 'wide.mp4', 'fifty.mp4', 'hundred.mp4', 'welcome.png'
    ]
    success = 0; failed = []
    for filename in all_files:
        if filename in FILE_ID_CACHE: continue 
        path = get_media_path(filename)
        if not path: failed.append(filename); continue
        try:
            with open(path, 'rb') as f:
                if filename.endswith(('.png', '.jpg', '.jpeg')):
                    msg = await bot.send_photo(LOG_CHANNEL_ID, f, caption=f"☁️ Caching {filename}")
                    fid = msg.photo[-1].file_id
                else:
                    msg = await bot.send_video(LOG_CHANNEL_ID, f, caption=f"☁️ Caching {filename}")
                    fid = msg.video.file_id
                FILE_ID_CACHE[filename] = fid
                try: await db.media.update_one({"filename": filename}, {"$set": {"file_id": fid}}, upsert=True)
                except Exception: pass
                success += 1
            await asyncio.sleep(1.5) 
        except Exception as e: failed.append(filename); print(f"Error uploading {filename}: {e}")
    text = f"✅ <b>Media Caching Complete!</b>\n\n☁️ MongoDB में सेव हुई फाइल्स: <b>{success}</b>\n❌ नहीं मिली फाइल्स: <b>{len(failed)}</b>"
    if failed: text += f"\n\n<b>Missing/Failed Files:</b>\n{', '.join(failed)}"
    await bot.reply_to(message, text, parse_mode='HTML')
