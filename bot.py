...
# ==========================================
# 5. DATA FORMATTERS & UI GENERATORS
# ==========================================
...
def live_score_text(game: Game) -> str:
    ...
    zero_left = max(0, zero_limit(game) - game.current_over_zeros)
    b_txt = f"{mention_user(batter)} — {batter.runs} runs ({batter.balls} balls)" if batter else "—"
    bl_txt = mention_user(bowler) if bowler else "—"
    return (f"{pe('🏏')} <b>Live Match Score</b>\n\n🔵 <b>Batter:</b> {b_txt}\n🎯 <b>Bowler:</b> {bl_txt}\n⚾ <b>Over:</b> <b>{game.current_over_balls}/{over_length(game)} balls</b>\n0️⃣ <b>Zeros left:</b> <b>{zero_left}</b>")
...
async def end_game(chat_id: int):
    ...
    txt_score = format_scoreboard_text(game)
    ...
    asyncio.create_task(update_stats_mongo(db_summary, 'tour' if game.is_special else 'solo'))
...
async def begin_game(chat_id: int):
    ...
    mode_note = f"<i>6-ball overs · Bowler rotates every over · Everyone bats & bowls</i> 🏟️" if game.is_special else f"<i>{game.over_balls}-ball overs · Zero max {zero_limit(game)} per over · Batter types 0–6 in the chat!</i> 🤫"

    text = f"{pe('🏏')} <b>{'Stars Special Match' if game.is_special else 'Solo Hand Cricket'} — Game On!</b>\n\n<b>Batting Order:</b>\n{order_lines}\n\n{mode_note}"
...
async def addsolo_cmd(message):
    ...
    # ❌ BLOCK FOR TOURNAMENT / SPECIAL MATCHES
    if game.is_special or getattr(game, 'is_tour_match', False):
