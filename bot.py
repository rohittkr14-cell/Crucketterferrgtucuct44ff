...
# ==========================================
load_dotenv()
BOT_TOKEN = "8647892164:AAFyPAQDpxs9Td9y2bgavfni4-ZERXZyzbY"
...
# ==========================================
# 5. DATA FORMATTERS & RETRO LEADERBOARD
# ==========================================
...
def build_bowler_list(game: Game) -> List[int]:
    ...
    # 🔥 Exclude eliminated players from bowling
...
async def cb_bowl(call):
    ...
    # WIDE LOGIC
    if game.is_special or game.is_tour_match:
...
async def end_game(chat_id: int):
    ...
    async def save_match_data():
        ...
        for p in game.players:
            ...
            if game.is_tour_match and chat_id in tours:
                tour = tours[chat_id]
...
async def start_cmd(message):
    ...
    if message.chat.id in tours: return await bot.reply_to(message, "⚠️ <b>A Tour is already running in this group!</b>\nEnd or pause it first to play normal matches.", parse_mode='HTML')
    if has_game(message.chat.id): return await bot.reply_to(message, '⚠️ A game is already running! Use /cancel to end it first.')
...
# ==========================================
# 13. EXCLUSIVE NEW COMMANDS (OGW, OGP, STARTSOLO, ADDSOLO, REMOVEGROUP, REMOVEPLAYER)
# ==========================================
...
async def addsolo_cmd(message):
    ...
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    msg = await bot.reply_to(message, f"⚡ <b>{html.escape(target.first_name)}</b>, do you want to join the current match?", parse_mode='HTML', reply_markup=kbd)
...
async def cb_addsolo(call):
    ...
    if game.status == 'waiting':
        ...
        batter = get_batter(game)
        if batter and batter.id != target_id:
            game.bowler_list.append(target_id)
...
async def cb_help_guide(call):
    ...
    if guide_type == 'normal':
        ...
        txt = ("🏆 <b>Special Match Guide (Admin Only)</b>\n\n"
               ...
               "• <code>/startmatch</code> : Forces the match to begin.\n"
...
async def cb_bowl(call):
    ...
    # Reset Timeout Count on valid play
    game.bowler_timeout_count[bowler.id] = 0
    if game.bowler_timer_task: game.bowler_timer_task.cancel()
    game.pending_bowl = num; game.awaiting_bat = True
    try: await bot.answer_callback_query(call.id, '🔒 Locked!')
...
# ==========================================
# 16. MAIN LAUNCHER & MENU INJECTOR
# ==========================================
async def set_bot_commands():
    # Telegram UI Menu List
    cmds = [
