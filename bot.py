...
# ==========================================
load_dotenv()
BOT_TOKEN = "8647892164:AAEh-TG1451xNpwcAcQXCNI1MfzD7bivj8E"

bot = AsyncTeleBot(BOT_TOKEN, parse_mode='HTML') 
bot_username = ""
BOT_ID = 0  # Global variable for Bot ID
...
COMMENTARY = {
    ...
    'hi': {
        0: ['गोटे मुँह में आ गए थे! बच गया... शानदार डिफेंस।', 'डॉट बॉल! बॉलर ने पूरी तरह चकमा दे दिया।', 'खाता नहीं खुल रहा, बॉलर भारी पड़ रहा है!'],
        1: ['हल्के हाथ से खेला और चतुराई से सिंगल निकाला।', 'स्ट्राइक रोटेट कर रहे हैं, स्मार्ट प्ले।', 'एक रन चुरा लिया!'],
        2: ['तेज़ दौड़! फील्डर देखते रह गए और दो रन निकाल लिए।', 'क्या रनिंग है भाई, चीता फेल!', 'गैप में धकेला और आसानी से दो रन पूरे किए।'],
        ...
        6: ['SIX! गेंद सीधा चाँद पर! बॉलर रो रहा है।', 'आसमान को चूमता हुआ लाजवाब छक्का!', 'स्टेडियम के बाहर! क्या ताक़त है इस शॉट में!']
    }
}
...
def out_commentary(lang: str) -> str: 
    if lang == 'hi': return "गिल्ली उड़ गई! बॉलर ने पूरी तरह चकमा दे दिया। OUT!"
    return "The bowler completely bamboozled the batter! WICKET!"

# ==========================================
# PFP FETCHING UTILITY
# ==========================================
async def fetch_pfp(user_id: int) -> Optional[bytes]:
    try:
...
def live_score_text(game: Game) -> str:
    ...
    if getattr(game, 'is_bot_match', False): mode_name = "GROUP 1v1 vs BOT"
    if game.over_balls == 9999: overs_txt = "Unlimited Overs"
...
# 🎴 IMPROVED VISUAL PLAYER CARD GENERATOR (NEW 8-BOX GRID + PFP + CUSTOM BG OVERLAYS)
def generate_stats_card_image(s: dict, game_type: str, theme: str = "default", pfp_bytes: bytes = None) -> Optional[io.BytesIO]:
    ...
    try:
        ...
        # Color Themes Setup 🔥
        if theme == 'spider':
            bg_color = (15, 5, 5); accent1 = (255, 50, 50); accent2 = (100, 20, 20); text_main = (255, 255, 255); box_bg = (30, 10, 10); box_out = (255, 50, 50)
        elif theme == 'dark':
        ...
        else: # Default Theme matched exactly to provided image
            bg_color = (238, 244, 250); accent1 = (59, 130, 246); accent2 = (191, 219, 254); text_main = (30, 41, 59); box_bg = (255, 255, 255); box_out = (191, 219, 254)

        img = Image.new('RGB', (width, height), color=bg_color)
        
        # 🟢 Custom Image Loader Logic
        custom_loaded = False
        bg_path = get_media_path(f"{theme}_theme.jpg") or get_media_path(f"{theme}_theme.png")
        if not bg_path: bg_path = get_media_path(f"{theme}.jpg") or get_media_path(f"{theme}.png")
        
        if bg_path:
            try:
                custom_bg = Image.open(bg_path).convert('RGB').resize((width, height))
        ...
        try:
            f_xl = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 50)
            f_l = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 40)
            f_m = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 30)
            f_s = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 22)
        except:
            f_xl = f_l = f_m = f_s = ImageFont.load_default()

        # Tech Lines Background (only draw if no custom background to avoid messing up images)
        if not custom_loaded:
            draw.line([(0, height-60), (width, height-60)], fill=accent2, width=2)
            draw.line([(0, 120), (350, 120)], fill=accent1, width=3)
            draw.line([(width-250, 60), (width, 60)], fill=accent2, width=2)

        # EXACT AVATAR ALIGNMENT
        avatar_x, avatar_y = 60, 40
        avatar_size = 160
        ...
        if pfp_bytes:
            try:
                ...
                img.paste(pfp_img, (avatar_x, avatar_y), mask)
            except Exception:
                pass
        
        # Outer ring for PFP locked exactly over the avatar
        draw.ellipse([(avatar_x, avatar_y), (avatar_x+avatar_size, avatar_y+avatar_size)], outline=accent1, width=6)
        
        name = s.get('username') or s.get('first_name', 'Player')
        draw.text((250, 75), f"@{name}".upper()[:15], font=f_xl, fill=text_main)
        draw.text((250, 140), f"{game_type.upper()} CAREER STATS", font=f_l, fill=accent1)
        draw.text((width-50, 100), f"ID: {s.get('user_id', 'N/A')}", font=f_m, fill=text_main, anchor="rm")
        ...
        # Larger 8 Boxes Config
        box_w = 210; box_h = 95
        x_start = 40; padding = 15
        y1 = 280; y2 = 400

        def draw_box(col, row_y, title, val):
            x = x_start + (col * (box_w + padding))
            # Semi-transparent box if custom loaded, else solid
            box_color = box_bg + (220,) if custom_loaded else box_bg + (255,)
            try: draw_overlay.rounded_rectangle([(x, row_y), (x+box_w, row_y+box_h)], radius=12, fill=box_color, outline=box_out, width=3)
            except AttributeError: draw_overlay.rectangle([(x, row_y), (x+box_w, row_y+box_h)], fill=box_color, outline=box_out, width=3)
            
            draw.text((x + box_w//2, row_y + 30), title, font=f_s, fill=text_main, anchor="mm")
            draw.text((x + box_w//2, row_y + 65), str(val), font=f_m, fill=accent1, anchor="mm")
        ...
        # Combine overlay and base image
        img = Image.alpha_composite(img.convert('RGBA'), overlay).convert('RGB')
        draw = ImageDraw.Draw(img)

        # Footer Name Updated 🔥
        draw.text((width-50, height-40), "Stars Solo ✨", font=f_m, fill=accent1, anchor="rm")
...
def generate_scoreboard_image(game: Game) -> Optional[io.BytesIO]:
    ...
    try:
        width = 1000
        height = 450 + (len(game.players) * 200)
        img = Image.new('RGB', (width, height), color=(8, 13, 17))
        draw = ImageDraw.Draw(img)
        try:
            f_xl = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 55)
            f_l = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 45)
            f_m = ImageFont.truetype("DejaVuSansMono.ttf", 38)
            f_s = ImageFont.truetype("DejaVuSansMono.ttf", 30)
        except:
            f_xl = f_l = f_m = f_s = ImageFont.load_default()

        title = f"[ ] {game.tour_group_name.upper()} FINAL SCORE [ ]" if game.is_special else "[ ] SOLO FINAL SCORE [ ]"
        draw.text((width//2, 60), title, font=f_xl, fill=(88, 255, 33), anchor="mm") 
        draw.line([(50, 110), (width-50, 110)], fill=(255, 33, 170), width=4) 

        y = 150
        for p in game.players:
            sr = f"{(p.runs / p.balls) * 100:.1f}" if p.balls > 0 else '0.0'
            b_balls = game.bowling_balls.get(p.id, 0); b_wkts = game.wickets_by_bowler.get(p.id, 0); b_runs = game.bowling_runs.get(p.id, 0)
            eco = f"{(b_runs / max(1,b_balls)) * 6:.1f}" if b_balls > 0 else '0.0'
            ...
            draw.text((70, y+55), f"Bat: 4s: {p.fours} | 6s: {p.sixes} | SR: {sr}", font=f_s, fill=(200, 200, 200))
            draw.text((70, y+100), f"Bowl: {b_balls}b | {b_wkts}W | {b_runs}R | Eco: {eco}", font=f_s, fill=(200, 200, 200))
            y += 200

        draw.line([(50, y), (width-50, y)], fill=(255, 33, 170), width=4)
        y += 40
        ...
        draw.text((50, y+55), f"Orange Cap: {ocn} ({orange_cap.runs if orange_cap else 0}R)", font=f_m, fill=(255, 170, 0))
        draw.text((50, y+105), f"Purple Cap: {pcn} ({purple_cap[1] if purple_cap else 0}W)", font=f_m, fill=(255, 33, 170))
        draw.text((50, y+160), f"Match Total: {total_runs} Runs in {overs_str} Overs", font=f_l, fill=(88, 255, 33))
...
def generate_leaderboard_image(title_text: str, records: list) -> Optional[io.BytesIO]:
    ...
    try:
        width = 1050
        height = 350 + (len(records) * 90)
        if height < 350: height = 350
        img = Image.new('RGB', (width, height), color=(8, 13, 17)) 
        draw = ImageDraw.Draw(img)
        try:
            f_title = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 75)
            f_hdr = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 40)
            f_l = ImageFont.truetype("DejaVuSansMono.ttf", 45)
            f_s = ImageFont.truetype("DejaVuSansMono.ttf", 35)
        except:
            f_title = f_hdr = f_l = f_s = ImageFont.load_default()

        draw.text((50, 40), "* HIGH SCORES *", fill=(255, 33, 170), font=f_hdr) 
        draw.text((50, 90), title_text, fill=(88, 255, 33), font=f_title) 

        draw.text((50, 200), "RANK", fill=(88, 255, 33), font=f_hdr)
        draw.text((200, 200), "PLAYER", fill=(88, 255, 33), font=f_hdr)
        draw.text((600, 200), "PTS", fill=(88, 255, 33), font=f_hdr)
        draw.text((760, 200), "RUNS", fill=(88, 255, 33), font=f_hdr)
        draw.text((900, 200), "WKTS", fill=(88, 255, 33), font=f_hdr)

        draw.line([(50, 250), (width-50, 250)], fill=(50, 70, 80), width=4)

        y = 280
        for i, r in enumerate(records):
            color = (255, 170, 0) if i < 3 else (220, 220, 220) 
            draw.text((50, y), f"{i+1:02d}", fill=color, font=f_l)
            name = str(r['name'])[:16] + ".." if len(str(r['name'])) > 16 else str(r['name'])
            draw.text((200, y), name, fill=color, font=f_l)
            draw.text((600, y), str(r['pts']), fill=color, font=f_l)
            draw.text((760, y), str(r['runs']), fill=(88, 255, 33) if i < 3 else (150, 200, 150), font=f_l)
            draw.text((900, y), str(r['wkts']), fill=(88, 255, 33) if i < 3 else (150, 200, 150), font=f_l)
            y += 90

        draw.text((width//2, y+50), "> INSERT COIN TO CONTINUE <", fill=(255, 33, 170), anchor="mm", font=f_hdr)
...
async def start_round(chat_id: int):
    ...
    # BOT AUTO-BOWLING LOGIC
    if getattr(game, 'is_bot_match', False) and bowler.id == BOT_ID:
        game.pending_bowl = random.randint(1, 6)
        game.awaiting_bat = True
        cap = f"{pe('⚡️')} {mention_user(batter)} — your turn!\n_Bot has locked in its delivery..._\n\nType your shot <b>(0–6)</b> in this group."
...
async def cb_bowl(call):
    ...
    back_kbd = None
    if game.group_link:
        rows = [[{"text": '🔙 Back to Game', "url": game.group_link}]]
        back_kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    ...
    # BOT AUTO-BATTING LOGIC
    if getattr(game, 'is_bot_match', False):
        batter = get_batter(game)
        if batter and batter.id == BOT_ID:
            game.pending_bat = random.randint(1, 6)
            game.awaiting_bat = False

    batter = get_batter(game)
    if batter and not getattr(game, 'is_bot_match', False): 
...
async def process_shot(chat_id: int):
    ...
    if bat == bowl:
        ...
        # Dual-Media Duck Logic Check
        if is_duck:
            await asyncio.sleep(1)
            duck_cap = f"🦆 <b>QUACK QUACK!</b>\n\n{mention_user(batter)} is out for a DUCK! Hilarious!"
            await send_media_with_fallback(chat_id, 'duck.mp4', duck_cap)
...
async def end_game(chat_id: int):
    ...
    for p in game.players:
        if p.id != BOT_ID:
            coin_msg += f"• {mention_user(p)} : {pe('🪙')} +{coin_rewards[p.id]}\n"
    ...
    db_summary = {
        ...
        } for p in game.players if p.id != BOT_ID],
...
# ==========================================
@bot.message_handler(commands=['botplay'])
async def botplay_cmd(message):
    run_safe_track(message)
    if message.chat.type == 'private': return await bot.reply_to(message, f"{pe('⚠️')} Use this command in a group to play against the Bot!")
    chat_id = message.chat.id
    if has_game(chat_id) or chat_id in tours: return await bot.reply_to(message, f"{pe('⚠️')} A Game or Tour is already running in this group! Use /cancel first.")
    
    game = Game(chat_id=chat_id, status='playing', host_id=message.from_user.id, is_special=False, over_balls=9999, created_at=datetime.now(timezone.utc))
    game.is_bot_match = True
    
    p1 = Player(id=message.from_user.id, username=message.from_user.username or "", first_name=message.from_user.first_name)
    p2 = Player(id=BOT_ID, username=bot_username, first_name="Bot")
    game.players = [p1, p2]
    ...
    order_lines = "\n".join([f"  {i+1}. {mention_user(get_player(game, id))}" for i, id in enumerate(game.batting_order)])
    text = f"{pe('🤖')} <b>Group 1v1 vs BOT</b>\n\n<b>Batting Order:</b>\n{order_lines}\n\n<i>Unlimited Overs · Play till Wicket!</i> ⚔️"
...
async def download_cmd(message):
    ...
    if len(parts) < 2: return await bot.reply_to(message, "Usage: `/download <filename.mp4/.png/.jpg>`", parse_mode='Markdown')
    filename = parts[1].strip()
    if not message.reply_to_message: return await bot.reply_to(message, f"{pe('⚠️')} Reply to a photo or video to save/update it.")
    ...
    if not file_id: return await bot.reply_to(message, f"{pe('❌')} No valid media found in the replied message.")
    
    await bot.reply_to(message, f"{pe('⏳')} Downloading and updating `{filename}`...", parse_mode='Markdown')
    try:
        ...
        MEDIA_PATH_CACHE[filename] = local_path
        # Update MongoDB
        await db.media.update_one({"filename": filename}, {"$set": {"file_id": file_id}}, upsert=True)
        
        await bot.reply_to(message, f"{pe('✅')} `{filename}` successfully updated in Local Storage & MongoDB!", parse_mode='Markdown')
    except Exception as e:
        await bot.reply_to(message, f"{pe('❌')} Error saving media: {e}")
...
async def buy_cmd(message):
    ...
    rows = [
        ...
        [{"text": '🕸️ Spider Theme', "callback_data": "preview_theme:spider_theme", "style": "primary"}],
        [{"text": '🖤 Dark Theme', "callback_data": "preview_theme:dark_theme", "style": "primary"}],
        [{"text": '🩵 Neon Theme', "callback_data": "preview_theme:neon_theme", "style": "primary"}],
        [{"text": '🌊 Ocean Theme', "callback_data": "preview_theme:ocean_theme", "style": "primary"}],
        [{"text": '🌲 Forest Theme', "callback_data": "preview_theme:forest_theme", "style": "primary"}]
    ]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    text = f"{pe('🛒')} <b>Welcome to Stars Shop</b>\n\nSelect an item to buy. Items are bought and applied instantly if you have enough coins!"
    await bot.reply_to(message, text, parse_mode='HTML', reply_markup=kbd)
...
async def stats_cmd(message):
    ...
    pfp = await fetch_pfp(message.from_user.id)
    img_buf = await asyncio.to_thread(generate_stats_card_image, s, 'solo', s.get('active_theme', 'default'), pfp)
...
async def tourstats_cmd(message):
    ...
    pfp = await fetch_pfp(message.from_user.id)
    img_buf = await asyncio.to_thread(generate_stats_card_image, s, 'tour', s.get('active_theme', 'default'), pfp)
...
async def join_cmd(message):
    run_safe_track(message)
    if message.chat.type == 'private': 
        return await bot.reply_to(message, f"{pe('⚠️')} Use this command in a group to join an open match lobby.")
    
    chat_id = message.chat.id
    u = message.from_user
    
    if is_banned(u.id): 
        return await bot.reply_to(message, f"{pe('🚫')} You are banned from playing for 5 minutes.")
        
    game = get_game(chat_id)
    if not game or game.status != 'waiting': 
        return await bot.reply_to(message, f"{pe('⚠️')} No open game lobby found here to join.")
        
    if game.is_special: 
        return await bot.reply_to(message, f"{pe('⚠️')} This is a Special Tour Match. Ask the Admin to manually add you.")
        
    if not game.over_balls: 
        return await bot.reply_to(message, f"{pe('⚠️')} The Host needs to select the Over Length first!")
        
    if get_player(game, u.id): 
        return await bot.reply_to(message, f"{pe('⚠️')} You have already joined the lobby!")
    ...
    await bot.reply_to(message, f"{pe('✅')} You joined the match successfully!")
    
    # Update Lobby Message Real-Time
    if game.lobby_message_id:
        opts = {"parse_mode": 'HTML', "reply_markup": lobby_keyboard(chat_id, False, game.over_balls)}
        try: 
            await bot.edit_message_caption(caption=lobby_text(game), chat_id=chat_id, message_id=game.lobby_message_id, **opts)
        except Exception:
            try: await bot.edit_message_text(text=lobby_text(game), chat_id=chat_id, message_id=game.lobby_message_id, **opts)
            except Exception: pass
...
async def cb_help_guide(call):
    ...
    if guide_type == 'normal':
        txt = ("📖 <b>Normal Match Guide</b>\n\n"
               ...
               "• <code>/join</code> : Use this command to join an open lobby too!\n"
               ...
               "• <code>/botplay</code> : Play Group 1v1 vs Bot (Unlimited Overs!)")
...
async def cb_statsview(call):
    ...
    await bot.answer_callback_query(call.id)
    try: 
        pfp = await fetch_pfp(user_id)
        img_buf = await asyncio.to_thread(generate_stats_card_image, s, view_type, s.get('active_theme', 'default'), pfp)
...
# ================================
# THEME PREVIEW & BUY SYSTEM
# ================================
@bot.callback_query_handler(func=lambda call: call.data.startswith("preview_theme:"))
async def cb_preview_theme(call):
    theme = call.data.split(":")[1]
    cost = 10000
    theme_clean = theme.replace('_theme', ' Theme').capitalize()
    
    # Dummy Stats for Preview using Actual User Data
    dummy_s = {
        'username': call.from_user.username or call.from_user.first_name, 
        'first_name': call.from_user.first_name, 
        'user_id': call.from_user.id,
        'matches': 50, 'total_runs': 1250, 'total_balls': 600, 'total_wickets': 45,
        'total_bowling_balls': 300, 'total_runs_conceded': 400, 'highest_score': 105, 'motms': 12
    }
    
    pfp = await fetch_pfp(call.from_user.id)
    theme_arg = theme.replace('_theme', '')
    img_buf = await asyncio.to_thread(generate_stats_card_image, dummy_s, 'solo', theme_arg, pfp)
    
    text = f"🎨 <b>Theme Preview: {theme_clean}</b>\n\nCost: <b>{cost} Coins</b>\n\nDo you want to buy this theme for your Stats Profile?"
    rows = [
        [{"text": "💳 Buy Now", "callback_data": f"buy_item:{theme}"}],
        [{"text": "🔙 Back", "callback_data": "back_shop"}]
    ]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    ...
    if img_buf:
        await bot.send_photo(call.message.chat.id, img_buf.getvalue(), caption=text, parse_mode='HTML', reply_markup=kbd)
    else:
        await bot.send_message(call.message.chat.id, text, parse_mode='HTML', reply_markup=kbd)
    await bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda call: call.data == "back_shop")
async def cb_back_shop(call):
    rows = [
        [{"text": 'Buy Teddy', "callback_data": "buy_item:teddy", "style": "primary"}],
        [{"text": 'Buy Heart', "callback_data": "buy_item:heart", "style": "primary"}],
        [{"text": '2x Coin Booster', "callback_data": "buy_item:booster", "style": "success"}],
...
async def cb_buy_item(call):
    ...
    # Auto Check DB
    u = await db.player_stats.find_one({"user_id": user_id})
    if u and item.endswith('_theme'):
        theme_name = item.replace('_theme', '')
        if theme_name in u.get("unlocked_themes", ["default"]):
            return await bot.answer_callback_query(call.id, f"⚠️ You already own the {theme_name.capitalize()} theme!", show_alert=True)
    ...
    if item == 'booster':
        await db.player_stats.update_one({"user_id": user_id}, {"$set": {"booster_expiry": datetime.now(timezone.utc) + timedelta(days=1)}})
        await db.coin_history.insert_one({"user_id": user_id, "amount": 0, "reason": "Bought 2x Coin Booster", "created_at": datetime.now(timezone.utc)})
        msg_txt = "🎉 You successfully bought the 2x Coin Booster! It has been applied to your account."
    elif item.endswith('_theme'):
        theme_name = item.replace('_theme', '')
        await db.player_stats.update_one({"user_id": user_id}, {"$addToSet": {"unlocked_themes": theme_name}})
        await db.player_stats.update_one({"user_id": user_id}, {"$set": {"active_theme": theme_name}})
        await db.coin_history.insert_one({"user_id": user_id, "amount": 0, "reason": f"Bought {theme_name.capitalize()} Theme", "created_at": datetime.now(timezone.utc)})
        msg_txt = f"🎉 You successfully bought the {theme_name.capitalize()} Theme! It has been automatically equipped."
        # Update Menu View
        await cb_back_shop(call)
    else:
        field = "teddy_count" if item == 'teddy' else "heart_count"
        await db.player_stats.update_one({"user_id": user_id}, {"$inc": {field: 1}})
        await db.coin_history.insert_one({"user_id": user_id, "amount": 0, "reason": f"Bought {item}", "created_at": datetime.now(timezone.utc)})
        msg_txt = f"🎉 You successfully bought a {item.capitalize()}!"
    
    await bot.answer_callback_query(call.id, "✅ Purchase Successful & Applied!", show_alert=True)
    try: await bot.send_message(user_id, msg_txt)
    except Exception: pass

@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_buy:"))
async def cb_admin_buy(call):
    # Maintained intentionally as requested
    if not is_bot_admin(call.from_user.id): return
    parts = call.data.split(":")
...
async def set_bot_commands():
    cmds = [
        ...
        BotCommand("join", "Join open lobby match in Group"),
        BotCommand("botplay", "Play Group 1v1 vs Bot"),
        ...
        BotCommand("download", "Fast Media Upload/Update (Owner)"),
