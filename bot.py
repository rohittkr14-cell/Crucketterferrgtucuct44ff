...
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton, BotCommand, InputMediaPhoto
...
# ==========================================
load_dotenv()
BOT_TOKEN = "8647892164:AAHeNRhk3ZxQNC9gd2GS_WobkC_Vkb8fCzk"
...
PREMIUM_EMOJIS = {
    ...
    "💥": "5465250797879057135", "⭐️": "5053473385355412667", "👎": "5334603549206653829",
    "🎯": "6032949275732742941", "🏀": "5935847413859225147", "🪙": "5145750530076706012", 
    "✅": "4976940882071651344", "💼": "5967389567781703494", "🧸": "5206502842478638898", 
    "❤️": "5406926593698312391"
}
THUMBS_UP_EMOJIS = ['<tg-emoji emoji-id="4915896438879159184">👍</tg-emoji>', '<tg-emoji emoji-id="5073456536743838369">👍</tg-emoji>']
...
def send_log(text: str): asyncio.create_task(_send_log_task(text))
...
async def check_owner_access(message) -> bool:
    ...
    try: await bot.reply_to(message, f"❌ <b>Only Owner can use this command.</b>")
    except Exception: pass
    return False

async def check_bot_admin_access(message) -> bool:
    if is_bot_admin(message.from_user.id): return True
    try: await bot.reply_to(message, f"❌ <b>You have no access.</b>")
    except Exception: pass
    return False
...
async def get_stats(user_id: int, game_type: str) -> Optional[dict]:
    return await db.player_stats.find_one({"user_id": user_id})

async def get_user(user_id: int):
    u = await db.player_stats.find_one({"user_id": user_id})
    if not u:
        u = {
            "user_id": user_id, "matches": 0, "motms": 0, "wins": 0, "losses": 0, "form": [],
            ...
            "total_runs_conceded": 0, "coins": 0, "teddy_count": 0, "heart_count": 0,
            "booster_expiry": None
        }
        await db.player_stats.insert_one(u)
    return u

async def update_stats_mongo(summary: dict, game_type: str):
    try:
        for p in summary['players']:
            u = await get_user(p['id'])
            u["username"] = p['username'] or u.get("username", "")
            u["first_name"] = p['first_name'] or u.get("first_name", "")
            u["matches"] = u.get("matches", 0) + 1
            won = (p['id'] == summary['winnerId'])
            if won: 
                u["wins"] = u.get("wins", 0) + 1
                u["form"] = (['W'] + u.get("form", []))[:5]
            else: 
                u["losses"] = u.get("losses", 0) + 1
                u["form"] = (['L'] + u.get("form", []))[:5]
            if p['id'] == summary['motmId']: u["motms"] = u.get("motms", 0) + 1
            u["highest_score"] = max(u.get("highest_score", 0), p['runs'])
            u["total_runs"] = u.get("total_runs", 0) + p['runs']
            u["total_balls"] = u.get("total_balls", 0) + p['balls']
            u["total_fours"] = u.get("total_fours", 0) + p['fours']
            u["total_sixes"] = u.get("total_sixes", 0) + p['sixes']
            if p['runs'] == 0 and p['balls'] > 0: u["ducks"] = u.get("ducks", 0) + 1
            if 50 <= p['runs'] < 100: u["fifties"] = u.get("fifties", 0) + 1
            if p['runs'] >= 100: u["hundreds"] = u.get("hundreds", 0) + 1
            u["total_wickets"] = u.get("total_wickets", 0) + p['wickets']
            u["total_bowling_balls"] = u.get("total_bowling_balls", 0) + p.get('bowling_balls', 0)
            u["total_runs_conceded"] = u.get("total_runs_conceded", 0) + p.get('bowling_runs', 0)
            
            earned = p.get('coins_earned', 0)
            # 🔥 BOOSTER LOGIC
            exp = u.get("booster_expiry")
            if exp and datetime.now(timezone.utc) < exp.replace(tzinfo=timezone.utc):
                earned *= 2
                
            if earned > 0:
                u["coins"] = u.get("coins", 0) + earned
            ...
            await db.player_stats.update_one({"user_id": p['id']}, {"$set": u})
...
async def process_admin_buy(user_id: int, item: str, action: str) -> bool:
    if action == 'app':
        ...
        await db.player_stats.update_one({"user_id": user_id}, {"$inc": {"coins": GIFT_CONFIG['amount']}})
        await db.coin_history.insert_one({"user_id": user_id, "amount": GIFT_CONFIG['amount'], "reason": f"Refund: {item} Rejected", "created_at": datetime.now(timezone.utc)})
    return True
...
# 🔥 AI BOT MODE DATACLASS
@dataclass
class BotGame:
    user_id: int; user_name: str; state: str = 'TOSS_OE'
    toss_call: str = ''; user_batting: bool = True
    inning: int = 1; user_runs: int = 0; bot_runs: int = 0
    user_balls: int = 0; bot_balls: int = 0; target: int = 0

games: Dict[int, Game] = {}; tours: Dict[int, Tour] = {}; ban_map: Dict[int, datetime] = {}
bot_games: Dict[int, BotGame] = {} # Tracks 1v1 Bot matches in DM
...
# ==========================================
# 4. MEDIA STRINGS & COMMENTARY FUNCTIONS
# ==========================================
RUN_FILES = {
...
COMMENTARY = {
    ...
    'hi': {
        0: ['शॉट मिस हुआ! बॉलर की शानदार गेंद।', 'डॉट बॉल! बल्लेबाज पूरी तरह चकमा खा गया।', 'कोई रन नहीं! बॉलर का पूरा दबाव है।'],
        1: ['हल्के हाथों से खेला और एक रन चुरा लिया।', 'सिंगल लेकर स्ट्राइक रोटेट की।', 'गैप में धकेला और 1 रन।'],
        2: ['तेज़ दौड़! 2 रन आसानी से पूरे किए!', 'फील्डर की खराब थ्रो, दो रन मिले।'],
        3: ['शानदार प्लेसमेंट और 3 रन भाग लिए!', 'चीते की रफ़्तार से 3 रन पूरे किए।'],
        4: ['चौका! गेंद गोली की तरह बाउंड्री के पार!', 'क्या कवर ड्राइव है! सीधा चार रन।'],
        5: ['5 रन! फील्डिंग टीम की बड़ी गलती।', 'ओवरथ्रो के रन! फ्री में 5 रन मिले।'],
        6: ['छक्काss! गेंद सीधा स्टेडियम के बाहर!', 'क्या गज़ब की पावर है, गेंद को चाँद पर भेज दिया!']
    }
}
...
def out_commentary(lang: str) -> str: 
    if lang == 'hi': return "गज़ब की गेंद! बल्लेबाज़ को पूरी तरह चकमा दिया। आउट!"
...
def live_score_text(game: Game) -> str:
    ...
    header = f"🏏 <b>{mode_name}</b>\n━━━━━━━━━━━━━━\n🎯 {overs_txt}  •  Players: {len(game.players)}\n━━━━━━━━━━━━━━\n\n"
    
    body = ""
    for idx, p in enumerate(game.players):
        marker = ""
        if get_batter(game) and p.id == get_batter(game).id: marker = "🏏 "
        elif get_bowler(game) and p.id == get_bowler(game).id: marker = "🎯 "
...
# 🔥 VISUAL PLAYER CARD GENERATOR
def generate_stats_card(s: dict, game_type: str) -> Optional[io.BytesIO]:
    if not HAS_PIL: return None
    try:
        img = Image.new('RGB', (600, 400), color=(15, 20, 25))
        draw = ImageDraw.Draw(img)
        try:
            f_xl = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 36)
            f_l = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 26)
            f_m = ImageFont.truetype("DejaVuSansMono.ttf", 22)
        except:
            f_xl = f_l = f_m = ImageFont.load_default()

        perf = s.get('total_runs', 0) + (s.get('total_wickets', 0) * 20) + (s.get('motms', 0) * 30)
        if perf >= 5000: tier_color = (0, 255, 255); tier_name = "DIAMOND"
        elif perf >= 2000: tier_color = (255, 215, 0); tier_name = "GOLD"
        elif perf >= 500: tier_color = (192, 192, 192); tier_name = "SILVER"
        else: tier_color = (205, 127, 50); tier_name = "BRONZE"

        draw.rectangle([(10, 10), (590, 390)], outline=tier_color, width=5)
        
        title = f"{game_type.upper()} CAREER STATS"
        draw.text((300, 40), title, font=f_xl, fill=(255, 255, 255), anchor="mm")
        draw.text((300, 80), f"Tier: {tier_name}", font=f_m, fill=tier_color, anchor="mm")
        
        name = s.get('username') or s.get('first_name', 'Player')
        draw.text((50, 120), f"Player: @{name}", font=f_l, fill=(88, 255, 33))
        ...
        total_balls = s.get('total_balls', 0)
        sr = f"{(runs / total_balls) * 100:.1f}" if total_balls > 0 else "0.0"
        
        draw.text((50, 180), f"Matches : {matches}", font=f_m, fill=(200, 200, 200))
        draw.text((350, 180), f"Runs    : {runs}", font=f_m, fill=(200, 200, 200))
        draw.text((50, 220), f"High Sc : {high}", font=f_m, fill=(200, 200, 200))
        draw.text((350, 220), f"Wickets : {wkts}", font=f_m, fill=(200, 200, 200))
        draw.text((50, 260), f"Str Rate: {sr}", font=f_m, fill=(200, 200, 200))
        draw.text((350, 260), f"MOTMs   : {motms}", font=f_m, fill=(200, 200, 200))
        
        form_arr = s.get('form', [])
        form_str = " ".join(["W" if r == 'W' else "L" for r in form_arr]) if form_arr else '—'
        draw.text((50, 320), f"Recent Form: {form_str}", font=f_l, fill=(255, 170, 0))
...
def format_stats_profile_html(s: dict, game_type: str) -> str:
    name = html.escape(f"@{s.get('username')}" if s.get('username') else s.get('first_name', 'Player'))
    mention = f'<a href="tg://user?id={s.get("user_id")}">{name}</a>'
    form_arr = s.get('form', [])
    ...
    header = f"{pe('📊')} <b>{game_type.upper()} CAREER PROFILE</b>"

    matches = s.get('matches', 0)
    avg = "0.00" if matches == 0 else f"{s.get('total_runs', 0) / matches:.2f}"
    total_balls = s.get('total_balls', 0)
    sr = "0.00" if total_balls == 0 else f"{(s.get('total_runs', 0) / total_balls) * 100:.2f}"
    b_balls = s.get('total_bowling_balls', 0)
    r_conceded = s.get('total_runs_conceded', 0)
    eco = "0.00" if b_balls == 0 else f"{(r_conceded / b_balls) * 6:.2f}"
    wkts = s.get('total_wickets', 0)
    b_avg = "—" if wkts == 0 else f"{r_conceded / wkts:.2f}"
    wins = s.get('wins', 0)
    losses = s.get('losses', 0)
    wr = "0.0" if matches == 0 else f"{(wins / matches) * 100:.1f}"

    return (f"{header}\n\n{pe('👤')} <b>Player:</b> {mention}\n"
            f"🆔 <b>ID:</b> <code>{s.get('user_id')}</code>\n{pe('📈')} <b>Form:</b> {form_str}\n────┈┄┄╌╌╌╌┄┄┈────\n\n"
            f"{pe('📊')} <b>MATCH STATS</b>\n{pe('🎮')} Matches: <b>{matches}</b> | {pe('🏆')} Highest: <b>{s.get('highest_score', 0)}</b>\n"
            f"{pe('⭐️')} MOMs: <b>{s.get('motms', 0)}</b> | {pe('⚡️')} Perf: <b>{perf}</b>\n\n"
            f"{pe('🏏')} <b>BATTING</b>\n{pe('🏃')} Runs: <b>{s.get('total_runs', 0)}</b> | {pe('📈')} Avg: <b>{avg}</b>\n"
            f"{pe('⚡️')} S/R: <b>{sr}</b> | {pe('💥')} 6s: <b>{s.get('total_sixes', 0)}</b> | 4s: <b>{s.get('total_fours', 0)}</b>\n"
            f"🔥 100s: <b>{s.get('hundreds', 0)}</b> | 50s: <b>{s.get('fifties', 0)}</b>\n\n"
            f"{pe('🎯')} <b>BOWLING</b>\n⚾ Wickets: <b>{wkts}</b>\n"
            f"{pe('🎯')} Econ: <b>{eco}</b> | {pe('📈')} Avg: <b>{b_avg}</b>\n\n"
            f"{pe('🧢')} <b>LEADERSHIP</b>\n{pe('↗️')} Win Rate: <b>{wr}%</b> ({pe('✅')} <b>{wins}</b> | ❌ <b>{losses}</b>)\n"
...
def zero_limit_message() -> str: return '❌ <b>Your limit has been reached for Zero. Play 1 to 6.</b>'
...
async def get_leaderboard_data(chat_id: int, tf: str):
    ...
    try:
        ...
        if not rows: return None, f"📭 No matches played in this group for selected timeframe."
        ...
        print(f"LBC Error: {e}")
        return None, "❌ Error loading leaderboard."
    ...
    try:
        ...
        if media_doc and "file_id" in media_doc:
            ...
            try:
                ...
                try: await db.media.delete_one({"filename": filename})
                except Exception: pass
    except Exception: pass
        ...
        try:
            with open(path, 'rb') as f:
                if filename.endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    try: await db.media.update_one({"filename": filename}, {"$set": {"file_id": msg.photo[-1].file_id}}, upsert=True)
                    except Exception: pass
                else:
                    msg = await bot.send_video(chat_id, f, caption=caption, parse_mode='HTML', reply_markup=reply_markup)
                    try: await db.media.update_one({"filename": filename}, {"$set": {"file_id": msg.video.file_id}}, upsert=True)
                    except Exception: pass
            return
        except Exception: pass
...
# ==========================================
# 8. AFK & TIMERS (Elimination & Penalty) 🔥
# ==========================================
async def kill_pending_buttons(chat_id: int, user_id: int, msg_id: int):
    try: await bot.edit_message_text(text="☠️ <b>Timeout! Button Dead.</b>", chat_id=user_id, message_id=msg_id, parse_mode='HTML')
    except Exception: pass
...
async def _lobby_timer_task(chat_id: int):
    try:
        ...
        try: await bot.send_message(chat_id, "⏳ <b>30 seconds left</b> to join the game! Hurry up.", parse_mode='HTML')
        except Exception: pass
        await asyncio.sleep(20)
        g = get_game(chat_id)
        if not g or g.status != 'waiting': return
        try: await bot.send_message(chat_id, "⏳ <b>10 seconds left</b> to join! Game starting soon.", parse_mode='HTML')
        except Exception: pass
        ...
        if len(g.players) < 2:
            delete_game(chat_id)
            try: await bot.send_message(chat_id, "❌ <b>Game Cancelled</b> — Only 1 player joined. Auto-cancelling.", parse_mode='HTML')
            except Exception: pass
        else:
...
async def _bowler_timer_task(chat_id: int):
    try:
        ...
        bl = get_bowler(g)
        if bl: 
            try: await bot.send_message(chat_id, f"⚠️ {mention_user(bl)} — <b>30s left!</b> Bowl NOW or face penalty (-6 runs & Free hit).", parse_mode='HTML')
            except Exception: pass
        ...
        bl = get_bowler(g)
        if bl: 
            try: await bot.send_message(chat_id, f"⚠️ {mention_user(bl)} — <b>10s left!</b> Hurry up!", parse_mode='HTML')
            except Exception: pass
...
async def _batter_timer_task(chat_id: int):
    try:
        ...
        bt = get_batter(g)
        if bt: 
            try: await bot.send_message(chat_id, f"⚠️ {mention_user(bt)} — <b>30s left!</b> Type your shot NOW or face penalty (-6 runs & Ball skipped).", parse_mode='HTML')
            except Exception: pass
        ...
        bt = get_batter(g)
        if bt: 
            try: await bot.send_message(chat_id, f"⚠️ {mention_user(bt)} — <b>10s left!</b> Hurry up!", parse_mode='HTML')
            except Exception: pass
...
def clear_all_round_timers(game: Game):
    ...
    if game.bowler_timer_task and game.bowler_timer_task != curr:
        game.bowler_timer_task.cancel()
    if game.batter_timer_task and game.batter_timer_task != curr:
        game.batter_timer_task.cancel()
...
async def eliminate_bowler(chat_id: int):
    try:
        ...
        if count == 1:
            ...
            try: await bot.send_message(chat_id, f"⚠️ <b>WARNING!</b> {mention_user(bowler)} you took too long to bowl!\n\n📉 Penalty: <b>-6 Runs</b> from your score!\n🎁 {mention_user(batter)} gets a <b>FREE HIT (+6 Runs)</b>!\n\n<i>Next timeout = Elimination!</i>", parse_mode='HTML')
            except Exception: pass
            ...
            game.pending_bowl = None; game.pending_bat = None; game.awaiting_bat = False
            try: await bot.send_message(chat_id, f"🚫 <b>ELIMINATED!</b>\n\n{mention_user(bowler)} failed to bowl AGAIN and is kicked out of the match!\n📉 Penalty: <b>-6 Runs</b>.\n🚫 Banned for <b>{mins} min</b>.", parse_mode='HTML')
            except Exception: pass
            ...
            if not game.bowler_list:
                ...
                return
            game.current_over_balls = 0
            game.current_over_zeros = 0
            nb, nbl = get_batter(game), get_bowler(game)
            if not nb or not nbl:
                await end_game(chat_id)
                return
            try: await bot.send_message(chat_id, f"🔄 <b>Bowler Eliminated! New Matchup:</b>\n\n🏏 Batting: {mention_user(nb)}\n🎯 Bowling: {mention_user(nbl)}", parse_mode='HTML')
            except Exception: pass
            await start_round(chat_id)
...
async def eliminate_batter(chat_id: int):
    try:
        ...
        if count == 1:
            ...
            try: await bot.send_message(chat_id, f"⚠️ <b>WARNING!</b> {mention_user(batter)} you took too long to bat!\n\n📉 Penalty: <b>-6 Runs</b>!\n⏩ Ball skipped (0 runs scored).\n\n<i>Next timeout = Elimination!</i>", parse_mode='HTML')
            except Exception: pass
            ...
            if game.current_over_balls >= over_length(game):
                ...
                nb = get_bowler(game)
                if nb:
                    try: await bot.send_message(chat_id, f"🔄 <b>Over complete ({over_length(game)} balls)!</b> New bowler: {mention_user(nb)}", parse_mode='HTML')
                    except Exception: pass
            await start_round(chat_id)
        else:
...
async def cb_runup(call):
    ...
    try: await bot.edit_message_text("🏏 <b>Bowling Started!</b>", call.message.chat.id, call.message.message_id, parse_mode='HTML')
    except Exception: pass
...
async def cb_bowl(call):
    ...
    try: await bot.answer_callback_query(call.id, '🔒 Locked!')
    except Exception: pass
...
async def advance_batter(chat_id: int):
    ...
    try: await bot.send_message(chat_id, f"🔄 <b>Next Innings!</b>\n\n🏏 Batting: {mention_user(nb)}\n🎯 Bowling: {mention_user(nbl)}", parse_mode='HTML')
    except Exception: pass
    await start_round(chat_id)
...
async def end_game(chat_id: int):
    ...
    coin_msg = f"\n💰 <b>Coins Earned:</b>\n"
    for p in game.players: coin_msg += f"• {mention_user(p)} : 🪙 +{coin_rewards[p.id]}\n"
    ...
    if str(SCORE_CHANNEL_ID).startswith("-100"):
        try:
            channel_caption = f"🏆 <b>Match Result</b>\nGroup ID: <code>{chat_id}</code>\nWinner: <b>{winner.first_name}</b>\n\n{txt_score}"
        ...
        except Exception as e: print(f"Score channel send failed: {e}")

async def begin_game(chat_id: int):
    game = get_game(chat_id)
    if not game or game.status != 'waiting': return
    if len(game.players) < 2:
        try: await bot.send_message(chat_id, '❌ Need at least 2 players to start!')
        except Exception: pass
        return
...
# ==========================================
# 10. AI BOT MODE (1V1 in DM) 🔥
# ==========================================
async def bot_game_over(chat_id: int, user_id: int, user_won: bool):
    if user_id not in bot_games: return
    game = bot_games[user_id]
    del bot_games[user_id]
    
    if user_won:
        msg = f"🎉 <b>YOU WON!</b>\n\nYou defeated the bot and earned <b>50 Coins!</b> 🪙\n\n👤 You: {game.user_runs}\n🤖 Bot: {game.bot_runs}"
        await add_coins(user_id, 50, "Bot Match Win")
    else:
        msg = f"💀 <b>YOU LOST!</b>\n\nThe bot defeated you. Better luck next time!\n\n🤖 Bot: {game.bot_runs}\n👤 You: {game.user_runs}"
        
    await bot.send_message(chat_id, msg, parse_mode='HTML')

@bot.message_handler(commands=['vsbot', 'playbot'])
async def playbot_cmd(message):
    run_safe_track(message)
    if message.chat.type != 'private':
        return await bot.reply_to(message, "⚠️ This mode is only available in DMs. Click here to play: @" + bot_username)
    
    user_id = message.from_user.id
    if user_id in bot_games:
        del bot_games[user_id] # reset
        
    bot_games[user_id] = BotGame(user_id=user_id, user_name=message.from_user.first_name)
    
    rows = [[{"text": "Odd", "callback_data": "bot_toss:odd", "style": "primary"}, 
             {"text": "Even", "callback_data": "bot_toss:even", "style": "success"}]]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    await bot.send_message(message.chat.id, "🤖 <b>AI BOT MATCH</b>\n\nLet's do the Toss! Choose Odd or Even:", parse_mode='HTML', reply_markup=kbd)

@bot.callback_query_handler(func=lambda call: call.data.startswith("bot_toss:"))
async def cb_bot_toss(call):
    user_id = call.from_user.id
    if user_id not in bot_games: return await bot.answer_callback_query(call.id, "No active bot game.", show_alert=True)
    
    choice = call.data.split(":")[1]
    bot_games[user_id].toss_call = choice
    bot_games[user_id].state = 'TOSS_NUM'
    
    try: await bot.edit_message_text(f"You chose <b>{choice.upper()}</b>.\n\nNow, type a number from 1 to 6 in the chat to toss!", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML')
    except Exception: pass
    await bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda call: call.data.startswith("bot_choice:"))
async def cb_bot_choice(call):
    user_id = call.from_user.id
    if user_id not in bot_games: return
    game = bot_games[user_id]
    
    choice = call.data.split(":")[1]
    game.user_batting = (choice == 'bat')
    game.state = 'PLAYING'
    
    action = "Bat" if game.user_batting else "Bowl"
    try: await bot.edit_message_text(f"You chose to <b>{action}</b> first.\n\n🏏 <b>Match Started!</b>\nType numbers (0-6) to play your turn.", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML')
    except Exception: pass
    await bot.answer_callback_query(call.id)
...
async def lbc_cmd(message):
    run_safe_track(message)
    if message.chat.type == 'private': return await bot.reply_to(message, "⚠️ Leaderboard is only for Groups.")
    try:
        ...
        await bot.reply_to(message, f"❌ Error loading Leaderboard: {e}")
...
async def bug_cmd(message):
    ...
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Usage: <code>/bug &lt;message&gt;</code>", parse_mode='HTML')
...
async def broadcast_cmd(message):
    ...
    if not text: return await bot.reply_to(message, "⚠️ Usage: <code>/broadcast &lt;your message&gt;</code>", parse_mode='HTML')
    await bot.reply_to(message, "📢 Broadcast initiated. This may take some time...")
    chats = await db.known_chats.find().to_list(length=None)
    success = 0
    for c in chats:
        try:
            await bot.send_message(c["chat_id"], f"📢 <b>BROADCAST MESSAGE</b>\n\n{text}", parse_mode='HTML')
...
async def qpromo_cmd(message):
    ...
    if len(parts) != 5: return await bot.reply_to(message, "⚠️ Usage: `/qpromo <CODE> <COINS> <USES> <HOURS>`", parse_mode='HTML')
    ...
    try:
        ...
        await bot.reply_to(message, f"✅ <b>Promo Code Created!</b>\nCode: <code>{code}</code>\nValue: {coins} Coins\nMax Uses: {uses}\nExpires in: {hours} hours.", parse_mode='HTML')
    except Exception as e:
...
async def promo_cmd(message):
    run_safe_track(message)
    parts = message.text.split()
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Send a valid promo code. Ex: `/promo VIP100`", parse_mode='HTML')
    code = parts[1].strip()
    user_id = message.from_user.id
    
    try:
        res_redeem = await db.promo_redeems.find_one({"code": code, "user_id": user_id})
        if res_redeem: return await bot.reply_to(message, "⚠️ You have already redeemed this promo code.")
        
        promo = await db.promo_codes.find_one({"code": code})
        if not promo: return await bot.reply_to(message, "❌ Invalid Promo Code.")
        if datetime.now(timezone.utc) > promo["expires_at"].replace(tzinfo=timezone.utc): return await bot.reply_to(message, "❌ This promo code has expired.")
        if promo["used_count"] >= promo["max_uses"]: return await bot.reply_to(message, "❌ This promo code is fully used up.")
        ...
        await bot.reply_to(message, f"🎉 <b>PROMO REDEEMED!</b>\n\nYou received <b>{promo['coins']} Coins!</b> 🪙", parse_mode='HTML')
    except Exception as e:
        await bot.reply_to(message, f"❌ DB Error: {e}")
...
async def addadmin_cmd(message):
    ...
    if not await check_owner_access(message): return
    parts = message.text.split()
    if len(parts) < 2 or not parts[1].isdigit(): return await bot.reply_to(message, "⚠️ Usage: <code>/addadmin &lt;User_ID&gt;</code>", parse_mode='HTML')
    u_id = int(parts[1])
    ...
    else: await bot.reply_to(message, f"⚠️ User is already an Admin.", parse_mode='HTML')
...
async def removeadmin_cmd(message):
    ...
    if not await check_owner_access(message): return
    parts = message.text.split()
    if len(parts) < 2 or not parts[1].isdigit(): return await bot.reply_to(message, "⚠️ Usage: `/removeadmin <User_ID>`", parse_mode='HTML')
    u_id = int(parts[1])
    ...
    else: await bot.reply_to(message, f"⚠️ User is not an admin.", parse_mode='HTML')

@bot.message_handler(commands=['coinhistory'])
async def coinhistory_cmd(message):
    run_safe_track(message)
    if not is_bot_admin(message.from_user.id): return
    if not message.reply_to_message: return await bot.reply_to(message, "⚠️ Please reply to a user's message to view their coin history.", parse_mode='HTML')
    ...
    try:
        ...
        if not history: return await bot.reply_to(message, f"📭 No coin history found for <b>{target_name}</b>.", parse_mode='HTML')
...
async def send_coins_cmd(message):
    ...
    if not message.reply_to_message:
        return await bot.reply_to(message, "⚠️ Reply to a user's message to send coins.")
    
    parts = message.text.split()
    if len(parts) < 2 or not parts[1].isdigit():
        return await bot.reply_to(message, "⚠️ Usage: `/send <amount>`", parse_mode='HTML')
        
    amount = int(parts[1])
    if amount < 10:
        return await bot.reply_to(message, "⚠️ Minimum transfer amount is 10 coins.")
        
    sender_id = message.from_user.id
    receiver = message.reply_to_message.from_user
    
    if sender_id == receiver.id:
        return await bot.reply_to(message, "⚠️ You cannot send coins to yourself.")
        
    sender_bal, _, _ = await get_user_balance(sender_id)
    if sender_bal < amount:
        return await bot.reply_to(message, f"❌ Insufficient balance! You have {sender_bal} coins.")
    ...
    if success:
        ...
        await bot.reply_to(message, f"✅ <b>Transfer Successful!</b>\n\n💸 Sent: {amount}\n📉 Tax (5%): {tax}\n🎁 Received by {html.escape(receiver.first_name)}: {final_amount} Coins", parse_mode='HTML')
        send_log(f"💸 {message.from_user.first_name} sent {amount} coins to {receiver.first_name}. (Tax: {tax})")

@bot.message_handler(commands=['host'])
async def host_cmd(message):
    run_safe_track(message)
    game = get_game(message.chat.id)
    if not game: return await bot.reply_to(message, "⚠️ No active game in this chat.")
    host_p = get_player(game, game.host_id)
    if host_p:
        await bot.reply_to(message, f"👑 <b>Current Game Host:</b> {mention_user(host_p)}", parse_mode='HTML')
    else:
        await bot.reply_to(message, f"👑 <b>Current Game Host ID:</b> <code>{game.host_id}</code>", parse_mode='HTML')
...
async def start_cmd(message):
        ...
        try:
            ...
            if game and game.status == 'playing':
                ...
                if bl and bl.id == message.from_user.id:
                    ...
                    return await bot.reply_to(message, "✅ Click 'Start Run-up' to bowl!")
        except Exception: pass

    if message.chat.type == 'private':
        if is_banned(message.from_user.id): return await bot.reply_to(message, "🚫 You are banned.", parse_mode='HTML')
        add_url = f"https://t.me/{bot_username}?startgroup=true"
        rows = [
            [{"text": 'Add Bot to Group', "url": add_url, "style": "success"}],
            [{"text": 'Play AI Match (1v1)', "callback_data": 'bot_ai_start', "style": "primary"}],
    ...
    if message.chat.id in tours:
        return await bot.reply_to(message, "⚠️ <b>A Tour is already running in this group!</b>\nEnd or pause it first to play normal matches.", parse_mode='HTML')
    if has_game(message.chat.id): 
        return await bot.reply_to(message, '⚠️ A game is already running! Use /cancel to end it first.')
...
async def buy_cmd(message):
    run_safe_track(message)
    rows = [
        [{"text": 'Buy Teddy (10K)', "callback_data": "buy_item:teddy", "style": "primary"},
         {"text": 'Buy Heart (10K)', "callback_data": "buy_item:heart", "style": "primary"}],
        [{"text": '⚡ 2x Coin Booster (3000)', "callback_data": "buy_item:booster", "style": "success"}]
    ]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    text = f"🛒 <b>Welcome to Stars Shop</b>\n\nSelect an item to buy. Boosters apply instantly!"
...
async def cancel_cmd(message):
    ...
    if game.is_special and not is_bot_adm: return await bot.reply_to(message, "❌ Only Bot Admins can cancel Special Matches.")
    if not game.is_special and not is_normal_admin and not is_bot_adm: return await bot.reply_to(message, "❌ Only Host or Group Admin can cancel Normal Matches.")
    ...
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    await bot.reply_to(message, '⚠️ <b>Are you sure you want to permanently cancel and delete the active match?</b>', parse_mode='HTML', reply_markup=kbd)
...
async def specialmatch_cmd(message):
    ...
    if message.chat.type == 'private': return
    if not await check_bot_admin_access(message): return
    if message.chat.id in tours: return await bot.reply_to(message, "⚠️ A Tour is active in this group.")
    if has_game(message.chat.id): return await bot.reply_to(message, '⚠️ Game already running! Use /cancel first.')
...
async def addplayer_cmd(message):
    ...
    if not_found: reply += f"❌ Not found: {', '.join(not_found)}\n"
    if game.lobby_message_id:
...
async def cm_cmd(message):
    ...
    def comm_kbd(c_id, en, lang):
        rows = [[{"text": "ON" if en else "OFF", "callback_data": f"cm:{c_id}:toggle", "style": "success" if en else "danger"}],
                [{"text": "English", "callback_data": f"cm:{c_id}:en", "style": "primary"}, {"text": "Hindi", "callback_data": f"cm:{c_id}:hi", "style": "primary"}]]
        return InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
...
# ==========================================
# 13. SPECIAL TOUR & NEW COMMANDS
# ==========================================
@bot.message_handler(commands=['ogw'])
async def ogw_cmd(message):
    run_safe_track(message)
    if message.chat.id not in tours: return await bot.reply_to(message, "⚠️ No active tour in this group.", parse_mode='HTML')
...
async def startsolo_cmd(message):
    ...
    if not game.is_special and not game.over_balls: return await bot.reply_to(message, "⚠️ Select over length first!")
    if len(game.players) < 2: return await bot.reply_to(message, "⚠️ Need at least 2 players.")
    asyncio.create_task(begin_game(chat_id))
...
async def addsolo_cmd(message):
    ...
    chat_id = message.chat.id
    game = get_game(chat_id)
    if not game: return await bot.reply_to(message, "⚠️ No game running.")
    
    if game.is_special or getattr(game, 'is_tour_match', False):
        return await bot.reply_to(message, "⚠️ <b>/addsolo</b> is only available in Normal Matches.", parse_mode='HTML')
        
    if message.from_user.id != game.host_id:
        return await bot.reply_to(message, "❌ Only the Match Host can use /addsolo.", parse_mode='HTML')
    
    if not message.reply_to_message: return await bot.reply_to(message, "⚠️ Reply to a user's message to add them.")
    target = message.reply_to_message.from_user
    
    if is_banned(target.id):
        return await bot.reply_to(message, "⚠️ This user is currently banned from playing for 5 minutes.", parse_mode='HTML')
        
    if get_player(game, target.id): return await bot.reply_to(message, "⚠️ Player is already in the game.")
    ...
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    msg = await bot.reply_to(message, f"⚡ <b>{html.escape(target.first_name)}</b>, do you want to join the current match? (60s left)", parse_mode='HTML', reply_markup=kbd)
...
async def newtour_cmd(message):
    ...
    if not is_bot_admin(message.from_user.id): return
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Usage: `/newtour <Tournament Name>`", parse_mode='HTML')
    tour_name = parts[1].strip()
    res = await db.open_tournaments.find_one({"name": tour_name})
    if res: return await bot.reply_to(message, "❌ This tournament name already exists!")
    TOUR_SETUP_STATE[message.from_user.id] = {"step": 1, "name": tour_name}
    await bot.reply_to(message, f"📝 <b>Tour Name:</b> {html.escape(tour_name)}\n\nNow, please send the <b>GC ID</b> (Group Chat ID) where this tournament will take place.", parse_mode='HTML')

@bot.message_handler(commands=['register'])
async def register_cmd(message):
    run_safe_track(message)
    if message.chat.type != 'private': return await bot.reply_to(message, "⚠️ Please use /register in my DM.")
    ...
    if not open_tours: return await bot.reply_to(message, rules + "🚫 <b>There is no any active tournament.</b>", parse_mode='HTML')
...
async def listtour_cmd(message):
    ...
    if len(parts) > 1:
        ...
        if not tour: return await bot.reply_to(message, "❌ Tournament not found.")
    else:
        tours_list = await db.open_tournaments.find().sort("_id", -1).limit(1).to_list(1)
        if not tours_list: return await bot.reply_to(message, "❌ No active tournaments found.")
        tour = tours_list[0]
        
    regs = await db.tour_registrations.find({"tour_id": tour["_id"]}).to_list(length=None)
    if not regs: return await bot.reply_to(message, f"📭 No registrations yet for <b>{html.escape(tour['name'])}</b>.", parse_mode='HTML')
...
async def endtour_cmd(message):
    ...
    if not is_bot_admin(message.from_user.id): return
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Usage: `/endtour <Tournament Name>`", parse_mode='HTML')
    t_name = parts[1].strip()
    res = await db.open_tournaments.delete_one({"name": t_name})
    if res.deleted_count > 0: await bot.reply_to(message, f"✅ Tournament <b>{html.escape(t_name)}</b> has been ended and removed from registrations.", parse_mode='HTML')
    else: await bot.reply_to(message, f"❌ No tournament found with name <b>{html.escape(t_name)}</b>.", parse_mode='HTML')
...
async def specialtour_cmd(message):
    ...
    if message.chat.type == 'private': return
    if not await check_bot_admin_access(message): return
    if message.chat.id in tours: return await bot.reply_to(message, "⚠️ A Tour is already running in this chat. Use /tourend to end it.")
    if has_game(message.chat.id): return await bot.reply_to(message, '⚠️ Game already running! Use /cancel first.')
...
async def addgroup_cmd(message):
    ...
    if not await check_bot_admin_access(message): return
    tour = tours[message.chat.id]
    if tour.status == 'locked': return await bot.reply_to(message, "❌ Tour is locked! Use <code>/touropen</code> to modify groups.", parse_mode='HTML')
    parts = message.text.split()
    ...
    if already_added: resp += f"\n⚠️ Already added in group: {', '.join(already_added)}"
...
async def removegroup_cmd(message):
    ...
    if not await check_bot_admin_access(message): return
    tour = tours[message.chat.id]
    if tour.status == 'locked': return await bot.reply_to(message, "❌ Tour is locked! Use <code>/touropen</code> first.", parse_mode='HTML')
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Usage: `/removegroup <Group Name>`", parse_mode='HTML')
    g_name = parts[1].strip()
    if g_name in tour.groups:
        del tour.groups[g_name]
        await bot.reply_to(message, f"✅ Group <b>{html.escape(g_name)}</b> removed from the tour.", parse_mode='HTML')
    else:
        await bot.reply_to(message, f"❌ Group <b>{html.escape(g_name)}</b> not found.", parse_mode='HTML')
...
async def removeplayer_cmd(message):
    ...
    if tour.status == 'locked': return await bot.reply_to(message, "❌ Tour is locked! Use <code>/touropen</code> first.", parse_mode='HTML')
    parts = message.text.split()
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Usage: `/removeplayer <ID/@username>`", parse_mode='HTML')
    ...
    if not target_id: return await bot.reply_to(message, "❌ User not found.")
    removed = False
    for g_name, players in tour.groups.items():
        for p in players:
            if p.id == target_id:
                ...
                break
        if removed: break
    if removed: await bot.reply_to(message, f"✅ Player removed from the tour.", parse_mode='HTML')
    else: await bot.reply_to(message, f"❌ Player is not in any group.", parse_mode='HTML')
...
async def tourplay_cmd(message):
    ...
    if not await check_bot_admin_access(message): return
    tour = tours[message.chat.id]
    if tour.status != 'locked': return await bot.reply_to(message, "❌ Please <code>/tourlock</code> the tour first.", parse_mode='HTML')
    rows = []
...
async def tourend_cmd(message):
    ...
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    await bot.reply_to(message, "⚠️ <b>Are you sure you want to end or pause the ENTIRE Tour?</b>", parse_mode='HTML', reply_markup=kbd)
...
async def tourcontinue_cmd(message):
    ...
    if not await check_bot_admin_access(message): return
    parts = message.text.split()
    if len(parts) < 2: return await bot.reply_to(message, "⚠️ Usage: `/tourcontinue <CODE>`", parse_mode='HTML')
    code = parts[1].strip()
    if message.chat.id in tours: return await bot.reply_to(message, "⚠️ A tour is already running here. End it first.")
    if has_game(message.chat.id): return await bot.reply_to(message, "⚠️ A game is currently running here. End it first.")
    
    obj = await load_and_resume(code, 'tour')
    if not obj: return await bot.reply_to(message, "❌ Invalid or expired Tour Resume Code.")
...
# ==========================================
@bot.callback_query_handler(func=lambda call: call.data == "bot_ai_start")
async def cb_bot_ai_start(call):
    user_id = call.from_user.id
    if user_id in bot_games: del bot_games[user_id] 
    bot_games[user_id] = BotGame(user_id=user_id, user_name=call.from_user.first_name)
    rows = [[{"text": "Odd", "callback_data": "bot_toss:odd", "style": "primary"}, {"text": "Even", "callback_data": "bot_toss:even", "style": "success"}]]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
    try: await bot.edit_message_text("🤖 <b>AI BOT MATCH</b>\n\nLet's do the Toss! Choose Odd or Even:", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML', reply_markup=kbd)
    except Exception: pass
    await bot.answer_callback_query(call.id)
...
async def cb_addsolo(call):
    ...
    game = get_game(chat_id)
    if action == "no":
        try: await bot.edit_message_text("❌ Request Cancelled.", chat_id, call.message.message_id)
        except Exception: pass
        return await bot.answer_callback_query(call.id, "Cancelled.")
    
    if not game:
        try: await bot.edit_message_text("⚠️ Game has already ended.", chat_id, call.message.message_id)
        except Exception: pass
        return
    ...
    if get_player(game, target_id):
        try: await bot.edit_message_text("⚠️ You are already in the game.", chat_id, call.message.message_id)
        except Exception: pass
        return
    ...
    if game.status == 'waiting':
        ...
        try: await bot.edit_message_text(f"✅ <b>{html.escape(call.from_user.first_name)}</b> joined the live match successfully!", chat_id, call.message.message_id, parse_mode='HTML')
        except Exception: pass
...
async def cb_buy_item(call):
    ...
    if item == "booster":
        ...
        if not success: return await bot.answer_callback_query(call.id, f"❌ Not enough coins! Need 3000.", show_alert=True)
        exp = datetime.now(timezone.utc) + timedelta(hours=24)
        await db.player_stats.update_one({"user_id": user_id}, {"$set": {"booster_expiry": exp}})
        try: await bot.send_message(call.message.chat.id, f"⚡ <b>2x COIN BOOSTER ACTIVATED!</b>\n\n{html.escape(call.from_user.first_name)}, for the next 24 hours, you will earn DOUBLE coins in every match!", parse_mode='HTML')
        except Exception: pass
        return await bot.answer_callback_query(call.id, "Booster Activated!", show_alert=True)
    
    if GIFT_CONFIG["status"] != "on": return await bot.answer_callback_query(call.id, "🚫 Out of stock!", show_alert=True)
    cost = GIFT_CONFIG["amount"]
    success = await deduct_coins(user_id, cost)
...
async def cb_tourstart(call):
    ...
    order_lines = "\n".join([f"  {i+1}. {mention_user(get_player(game, id))}" for i, id in enumerate(game.batting_order)])
    await bot.send_message(chat_id, f"🏏 <b>Tour Match: Group {html.escape(grp_name)}!</b>\n\n<b>Batting Order:</b>\n{order_lines}\n\n<i>6-ball overs · Auto-rotating bowlers</i> 🏟️", parse_mode='HTML')
    await start_round(chat_id)
...
async def cb_statsview(call):
    ...
    view_type = parts[2]
    user_id = int(parts[1])
    s = await get_stats(user_id, 'solo' if view_type=='overall' else view_type)
    ...
    img_buf = generate_stats_card(s, view_type)
    if img_buf:
        media = InputMediaPhoto(img_buf.getvalue(), caption="Here are your stats!")
        try: await bot.edit_message_media(media=media, chat_id=call.message.chat.id, message_id=call.message.message_id)
        except Exception: pass
    else:
        rows = [
        ...
        kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
        try: await bot.edit_message_text(format_stats_profile_html(s, view_type), chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML', reply_markup=kbd)
        except Exception: pass
    await bot.answer_callback_query(call.id)
...
async def cb_cancel_req(call):
    ...
    delete_game(chat_id)
    try: await bot.edit_message_text(text='❌ <b>Game deleted forever.</b>', chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML')
    except Exception: pass
...
async def cb_cancel_lobby(call):
    ...
    delete_game(chat_id)
    try:
        if call.message.content_type == 'photo': await bot.edit_message_caption(caption='❌ <b>Lobby cancelled.</b>', chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML')
        else: await bot.edit_message_text(text='❌ <b>Lobby cancelled.</b>', chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='HTML')
    except Exception: pass
...
async def cb_admin_buy(call):
    ...
    try:
        if action == 'app': await bot.send_message(user_id, f"🎉 <b>Congratulations!</b>\n\nYour request for <b>{item.capitalize()}</b> has been ✅ <b>Approved</b>! Check /balance", parse_mode='HTML')
        else: await bot.send_message(user_id, f"😔 <b>Request Rejected!</b>\n\nYour Coins have been refunded.", parse_mode='HTML')
    except Exception: pass
...
async def handle_all_messages(message):
    ...
    if message.chat.type == 'private' and message.from_user.id in WAITING_FOR_GIFT_AMOUNT:
        ...
        if message.text and message.text.strip().lower() == 'skip':
            return await bot.reply_to(message, "✅ Skipped. Amount not changed.")
        elif message.text and message.text.strip().isdigit():
            GIFT_CONFIG['amount'] = int(message.text.strip())
            return await bot.reply_to(message, f"✅ Shop amount updated to {GIFT_CONFIG['amount']} Coins.")
        else:
            return await bot.reply_to(message, "❌ Invalid amount. Skipped.")
    
    if message.from_user.id in TOUR_SETUP_STATE and message.chat.type == 'private':
        state = TOUR_SETUP_STATE[message.from_user.id]
        if state["step"] == 1:
            if not message.text: return await bot.reply_to(message, "⚠️ Send GC ID (e.g., -100123456).")
            try:
                gc_id = int(message.text.strip())
                state["gc_id"] = gc_id
                state["step"] = 2
                await bot.reply_to(message, "✅ GC ID Saved.\nNow send a <b>Banner Photo</b> for the Tournament, or send <code>skip</code> to proceed without banner.", parse_mode='HTML')
            except ValueError:
                await bot.reply_to(message, "❌ Invalid ID. Send only numbers.")
            return
            ...
            else: return await bot.reply_to(message, "⚠️ Send a photo or send <code>skip</code>.", parse_mode='HTML')
    ...
    if message.chat.type == 'private' and message.from_user.id in WAITING_FOR_ADS:
        ...
        await bot.reply_to(message, "📢 Broadcasting your message everywhere. This will take some time...")
        try:
            chats = await db.known_chats.find().to_list(length=None)
            success = 0
            for c in chats:
    ...
    # 🔥 BOT GAME (PLAYBOT) INPUT HANDLER 🔥
    if message.chat.type == 'private' and message.from_user.id in bot_games:
        user_id = message.from_user.id
        bg = bot_games[user_id]
        
        if bg.state == 'TOSS_NUM':
            if not message.text or not message.text.isdigit(): return
            u_num = int(message.text.strip())
            if u_num < 1 or u_num > 6: return await bot.reply_to(message, "⚠️ Enter a number between 1 and 6.")
            
            b_num = random.randint(1, 6)
            total = u_num + b_num
            user_won_toss = False
            
            if total % 2 == 0 and bg.toss_call == 'even': user_won_toss = True
            elif total % 2 != 0 and bg.toss_call == 'odd': user_won_toss = True
            
            res_txt = f"You played <b>{u_num}</b>, Bot played <b>{b_num}</b>.\nSum = {total} (<b>{'Even' if total%2==0 else 'Odd'}</b>)\n\n"
            if user_won_toss:
                bg.state = 'TOSS_CHOICE'
                rows = [[{"text": "Batting", "callback_data": "bot_choice:bat", "style": "primary"}, {"text": "Bowling", "callback_data": "bot_choice:bowl", "style": "danger"}]]
                kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
                await bot.reply_to(message, res_txt + "🎉 <b>You won the Toss!</b> Choose Batting or Bowling:", parse_mode='HTML', reply_markup=kbd)
            else:
                bg.state = 'PLAYING'
                bg.user_batting = random.choice([True, False])
                action = "Bowl" if bg.user_batting else "Bat" # if user is batting, bot chose to bowl
                await bot.reply_to(message, res_txt + f"🤖 <b>Bot won the Toss</b> and chose to <b>{action}</b> first!\n\n🏏 <b>Match Started!</b>\nType numbers (0-6) to play.", parse_mode='HTML')
            return
            
        elif bg.state == 'PLAYING':
            if not message.text or not message.text.isdigit(): return
            u_num = int(message.text.strip())
            if u_num < 0 or u_num > 6: return await bot.reply_to(message, "⚠️ Play between 0 and 6.")
            
            b_num = random.randint(0, 6)
            bat_num = u_num if bg.user_batting else b_num
            bowl_num = b_num if bg.user_batting else u_num
            
            if bat_num == bowl_num:
                if bg.inning == 1:
                    bg.inning = 2
                    bg.user_batting = not bg.user_batting
                    bg.target = (bg.user_runs if not bg.user_batting else bg.bot_runs) + 1
                    msg = f"💥 <b>WICKET!</b> {'You' if not bg.user_batting else 'Bot'} played {b_num}.\n\nEnd of Innings 1!\n🎯 Target: <b>{bg.target}</b> runs.\n\nType 0-6 to start Innings 2."
                    await bot.reply_to(message, msg, parse_mode='HTML')
                else:
                    await bot_game_over(message.chat.id, user_id, not bg.user_batting)
            else:
                if bg.user_batting:
                    bg.user_runs += bat_num
                    bg.user_balls += 1
                else:
                    bg.bot_runs += bat_num
                    bg.bot_balls += 1
                ...
                if bg.inning == 2:
                    current_runs = bg.user_runs if bg.user_batting else bg.bot_runs
                    if current_runs >= bg.target:
                        await bot_game_over(message.chat.id, user_id, bg.user_batting)
                        return
                        
                u_score = f"{bg.user_runs}/{bg.user_balls}"
                b_score = f"{bg.bot_runs}/{bg.bot_balls}"
                t_str = f" | 🎯 Target: {bg.target}" if bg.inning == 2 else ""
                
                msg = f"You played <b>{u_num}</b>, Bot played <b>{b_num}</b>.\n🏏 Runs scored: <b>{bat_num}</b>\n\n👤 You: <b>{u_score}</b> | 🤖 Bot: <b>{b_score}</b>{t_str}"
                await bot.reply_to(message, msg, parse_mode='HTML')
            return

    # 🔥 GROUP GAME INPUT HANDLER 🔥
    if message.chat.type != 'private':
        chat_id = message.chat.id
        game = get_game(chat_id)
...
# ==========================================
# 17. MAIN LAUNCHER & FAKE PORT & MENU
# ==========================================
...
async def set_bot_commands():
    cmds = [
        ...
        BotCommand("playbot", "Play 1v1 vs Bot (DM)"),
        ...
        BotCommand("buy", "Buy Premium Items & Boosters"),
        BotCommand("score", "Check Live Match Score"),
        BotCommand("host", "Check current match host"),
        BotCommand("stats", "Check your Career Stats Card"),
        BotCommand("tourstats", "Check your Tour Stats Card"),
