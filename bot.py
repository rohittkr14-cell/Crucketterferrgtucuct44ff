...
def generate_stats_card_image(s: dict, game_type: str, theme: str = "default", pfp_bytes: bytes = None) -> Optional[io.BytesIO]:
    ...
    try:
        ...
        if bg_path:
            try:
                # Custom Templates scale perfectly to 1280x720 for crisp alignment
        ...
        if custom_loaded:
            try:
                f_xl = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 35)
                f_m = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 25)
                f_val = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 32)
            except:
                f_xl = f_m = f_val = ImageFont.load_default()

            # Perfect PFP Overlay for 1280x720 Template
            avatar_size = 260
            avatar_x, avatar_y = 135, 95
            ...
            draw.text((450, 150), f"@{name}".upper()[:15], font=f_xl, fill=(255,255,255))
            draw.text((945, 305), f"{s.get('user_id', 'N/A')}", font=f_m, fill=(255,255,255))
            
            # Perfect Centers for Dynamic Values Over Pre-Drawn Boxes
            draw.text((215, 480), str(matches), font=f_val, fill=(255,50,50), anchor="mm")
            draw.text((498, 480), str(runs), font=f_val, fill=(255,50,50), anchor="mm")
            draw.text((782, 480), str(sr), font=f_val, fill=(255,50,50), anchor="mm")
            draw.text((1065, 480), str(wkts), font=f_val, fill=(255,50,50), anchor="mm")
            
            draw.text((215, 660), str(eco), font=f_val, fill=(255,50,50), anchor="mm")
            draw.text((498, 660), str(high), font=f_val, fill=(255,50,50), anchor="mm")
            draw.text((782, 660), str(s.get('motms', 0)), font=f_val, fill=(255,50,50), anchor="mm")
            draw.text((1065, 660), str(perf), font=f_val, fill=(255,50,50), anchor="mm")
            ...
            if pfp_bytes:
                try:
                    pfp_img = Image.open(io.BytesIO(pfp_bytes)).convert("RGBA")
                    pfp_img = pfp_img.resize((150, 150))
                    mask = Image.new("L", (150, 150), 0)
                    draw_mask = ImageDraw.Draw(mask)
                    draw_mask.ellipse((0, 0, 150, 150), fill=255)
                    img.paste(pfp_img, (75, 45), mask)
                except Exception:
                    pass
            
            draw.ellipse([(70, 40), (230, 200)], outline=accent1, width=6)
            
            name = s.get('username') or s.get('first_name', 'Player')
            draw.text((260, 80), f"@{name}".upper()[:15], font=f_xl, fill=text_main)
            draw.text((260, 150), f"{game_type.upper()} CAREER STATS", font=f_l, fill=accent1)
...
def generate_scoreboard_image(game: Game) -> Optional[io.BytesIO]:
    ...
    try:
        width = 850
        height = 400 + (len(game.players) * 160)
        img = Image.new('RGB', (width, height), color=(8, 13, 17))
        draw = ImageDraw.Draw(img)
        try:
            f_xl = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 42)
            f_l = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 34)
            f_m = ImageFont.truetype("DejaVuSansMono.ttf", 28)
            f_s = ImageFont.truetype("DejaVuSansMono.ttf", 22)
        except:
            f_xl = f_l = f_m = f_s = ImageFont.load_default()
        ...
        draw.line([(50, 100), (width-50, 100)], fill=(255, 33, 170), width=4) 
        ...
        for p in game.players:
            ...
            draw.text((70, y+45), f"Bat: 4s: {p.fours} | 6s: {p.sixes} | SR: {sr}", font=f_s, fill=(200, 200, 200))
            draw.text((70, y+80), f"Bowl: {b_balls}b | {b_wkts}W | {b_runs}R | Eco: {eco}", font=f_s, fill=(200, 200, 200))
            y += 160
        ...
        draw.text((50, y+45), f"Orange Cap: {ocn} ({orange_cap.runs if orange_cap else 0}R)", font=f_s, fill=(255, 170, 0))
        draw.text((50, y+85), f"Purple Cap: {pcn} ({purple_cap[1] if purple_cap else 0}W)", font=f_s, fill=(255, 33, 170))
        draw.text((50, y+135), f"Match Total: {total_runs} Runs in {overs_str} Overs", font=f_l, fill=(88, 255, 33))
...
def generate_leaderboard_image(title_text: str, records: list) -> Optional[io.BytesIO]:
    ...
    try:
        width = 950
        height = 320 + (len(records) * 80)
        if height < 350: height = 350
        img = Image.new('RGB', (width, height), color=(8, 13, 17)) 
        draw = ImageDraw.Draw(img)
        try:
            f_title = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 60)
            f_hdr = ImageFont.truetype("DejaVuSansMono-Bold.ttf", 30)
            f_l = ImageFont.truetype("DejaVuSansMono.ttf", 36)
            f_s = ImageFont.truetype("DejaVuSansMono.ttf", 28)
        except:
...
async def cb_bowl(call):
    ...
    back_kbd = None
    if game.group_link:
        rows = [[{"text": '🔙 Back to Game', "url": game.group_link, "style": "primary"}]]
...
async def botplay_cmd(message):
    ...
    # Strict Host Assignment to BOT_ID prevents anyone from using /addsolo
    game = Game(chat_id=chat_id, status='playing', host_id=BOT_ID, is_special=False, over_balls=9999, created_at=datetime.now(timezone.utc))
...
async def addsolo_cmd(message):
    ...
    if getattr(game, 'is_bot_match', False):
        return await bot.reply_to(message, f"{pe('⚠️')} <b>/addsolo</b> cannot be used in a Bot Match.", parse_mode='HTML')
...
async def cb_preview_theme(call):
    ...
    rows = [
        [{"text": "💳 Buy Now", "callback_data": f"buy_item:{theme}", "style": "success"}],
        [{"text": "🔙 Back", "callback_data": "back_shop", "style": "danger"}]
    ]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
