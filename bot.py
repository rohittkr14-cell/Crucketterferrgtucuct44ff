...
# ==========================================
# 5. DATA FORMATTERS & NEW 8-BOX THEMES 🔥
# ==========================================
...
# 🎴 8-BOX THEME GENERATOR (LIKE PROVIDED IMAGE) 🔥
def generate_stats_card_image(s: dict, game_type: str, theme: str = "default") -> Optional[io.BytesIO]:
    ...
    try:
        ...
        else: # Default (Light White/Blue Tech)
            bg_color = (245, 247, 250); accent1 = (80, 140, 255); accent2 = (180, 200, 255); text_main = (30, 40, 60); box_bg = (255, 255, 255); box_out = (180, 210, 255)
        ...
        try:
            ...
            f_s = ImageFont.truetype("DejaVuSansMono.ttf", 16)
        except:
        ...
        draw.text((250, 150), f"{game_type.upper()} CAREER", font=f_l, fill=accent1)
        ...
        cent_fif = f"{s.get('hundreds',0)} / {s.get('fifties',0)}"

        # 8 Boxes Config
        box_w = 200; box_h = 80
        x_start = 50; padding = 20
        y1 = 280; y2 = 380
        ...
        draw_box(2, y2, "100s / 50s", cent_fif)
        draw_box(3, y2, "RATING", perf)

        # Footer
        draw.text((width-50, height-30), "Cricket Mania ✨", font=f_m, fill=accent1, anchor="rm")
...
async def ai_cb_handler(call):
    ...
    if cmd == 'diff':
        ...
        out = (bat_num == bowl_num) # 🏏 UNLIMITED OVERS LOGIC!
        if not out:
...
async def buy_cmd(message):
    ...
    rows = [
        ...
        [{"text": '🕸️ Spider Theme (10k)', "callback_data": "buy_item:spider_theme", "style": "primary"},
         {"text": '🖤 Dark Theme (10k)', "callback_data": "buy_item:dark_theme", "style": "primary"}]
    ]
    kbd = InlineKeyboardMarkup.de_json({"inline_keyboard": rows})
